...
void Tag::onInit() {
  img_subscriber_ =
      nh_.subscribe("/galaxy_camera/galaxy_camera/image_raw", 1, &Tag::receiveFromCam, this);
  pnp_publisher_ = nh_.advertise<rm_msgs::TagMsgArray>("tag_pnp_publisher", 1);

  hsv_red_publisher_ =
      nh_.advertise<sensor_msgs::Image>("tag_red_hsv_publisher", 1);
  hsv_blue_publisher_ =
      nh_.advertise<sensor_msgs::Image>("tag_blue_hsv_publisher", 1);
  masked_red_publisher_ =
      nh_.advertise<sensor_msgs::Image>("tag_masked_red_publisher", 1);
  masked_blue_publisher_ =
      nh_.advertise<sensor_msgs::Image>("tag_masked_blue_publisher", 1);
  segmentation_publisher_ =
      nh_.advertise<sensor_msgs::Image>("tag_segmentation_publisher", 1);
  callback_ = boost::bind(&Tag::dynamicCallback, this, _1);
  server_.setCallback(callback_);

  distortion_coefficients_ = (cv::Mat_<double>(1,5)<<-0.211020, 0.153512, 0.000720, 0.000481, 0.000000);
  camera_matrix_ = (cv::Mat_<double>(3,3)<<1282.38615,    0.     ,  647.8863 ,
            0.     , 1283.49104,  520.10776,
            0.     ,    0.     ,    1.     );

  cv::Mat temp_A = cv::imread("/home/yamabuki/detect_ws/src/tag_detector/A.png",
                              cv::IMREAD_GRAYSCALE);
  cv::Mat temp_B = cv::imread("/home/yamabuki/detect_ws/src/tag_detector/B.png",
                              cv::IMREAD_GRAYSCALE);
  cv::Mat temp_C = cv::imread("/home/yamabuki/detect_ws/src/tag_detector/C.png",
                              cv::IMREAD_GRAYSCALE);
  cv::Mat temp_D = cv::imread("/home/yamabuki/detect_ws/src/tag_detector/D.png",
                              cv::IMREAD_GRAYSCALE);
  cv::Mat temp_E = cv::imread("/home/yamabuki/detect_ws/src/tag_detector/E.png",
                              cv::IMREAD_GRAYSCALE);
  ...
  cv::resize(binary_a, binary_a, cv::Size(40, 40), 0, 0, cv::INTER_NEAREST);
  cv::resize(binary_b, binary_b, cv::Size(40, 40), 0, 0, cv::INTER_NEAREST);
  cv::resize(binary_c, binary_c, cv::Size(40, 40), 0, 0, cv::INTER_NEAREST);
  cv::resize(binary_d, binary_d, cv::Size(40, 40), 0, 0, cv::INTER_NEAREST);
  cv::resize(binary_e, binary_e, cv::Size(40, 40), 0, 0, cv::INTER_NEAREST);
...
void Tag::receiveFromCam(const sensor_msgs::ImageConstPtr &image) {
  cv_image_ = boost::make_shared<cv_bridge::CvImage>(
      *cv_bridge::toCvShare(image, image->encoding));

  imgProcess();
  segmentation_publisher_.publish(cv_bridge::CvImage(std_msgs::Header(),
                                                     cv_image_->encoding,
                                                     cv_image_->image)
                                      .toImageMsg());
  //    getTemplateImg();
...
void Tag::dynamicCallback(tag_detector::dynamicConfig &config) {
  ...
  shape_thresh_ = config.shape_thresh;
}

rm_msgs::TagMsg Tag::pubMessage(const cv::Mat &rvec, const cv::Mat &tvec,
                                const int signal, const int color) {
  cv::Mat rotate_mat;
  cv::Rodrigues(rvec, rotate_mat);

  tf::Matrix3x3 tf_rotate_matrix(
      rotate_mat.at<double>(0, 0), rotate_mat.at<double>(0, 1),
      rotate_mat.at<double>(0, 2), rotate_mat.at<double>(1, 0),
      rotate_mat.at<double>(1, 1), rotate_mat.at<double>(1, 2),
      rotate_mat.at<double>(2, 0), rotate_mat.at<double>(2, 1),
      rotate_mat.at<double>(2, 2));

  //  tf::Matrix3x3 tf_rotate_matrix(1, 0, 0, 0, 1, 0, 0, 0, 1);
  tf::Vector3 tf_tvec(tvec.at<double>(0, 0), tvec.at<double>(0, 1),
                      tvec.at<double>(0, 2));
  tf::Quaternion quaternion;
  double r;
  double p;
  double y;
  rm_msgs::TagMsg tag_msg;
  ...
  tf::StampedTransform stamped_Transfor(transform, ros::Time::now(),
                                        "oak_rgb_camera_optical_frame",
                                        "vis_tag" + std::to_string(signal));
  static tf::TransformBroadcaster broadcaster;
  broadcaster.sendTransform(stamped_Transfor);
  return tag_msg;
}

rm_msgs::TagMsg Tag::resultVisualizaion(const std::vector<cv::Point2i> &hull,
                                        const cv::Point2f (&vertex)[4],
                                        const int angle, const int signal,
                                        const int color) {
    if (signal == 6)
    {
  ...
  if (angle <= 0) {
    ...
    for (int i = 0; i < 4; i++) {
      cv::line(cv_image_->image, vertex[i], vertex[(i + 1) % 4],
               cv::Scalar(0, 255, 0), 3);
      cv::putText(cv_image_->image, std::to_string(i), vertex[i], 1, 3,
                  cv::Scalar(0, 255, 0), 3);
      p_points_vec.push_back(vertex[i]);
    }
    cv::Mat rvec;
    cv::Mat tvec;
    cv::solvePnP(w_points_vec, p_points_vec, camera_matrix_,
                 distortion_coefficients_, rvec, tvec, bool(),
                 cv::SOLVEPNP_ITERATIVE);
    auto tag_msg = pubMessage(rvec, tvec, signal, color);
    cv::circle(cv_image_->image, centroid, 2, cv::Scalar(0, 255, 0), 2);
    cv::putText(cv_image_->image, std::to_string(signal), centroid, 1, 3,
                cv::Scalar(255, 0, 0), 3);
    cv::putText(cv_image_->image, std::to_string(tvec.at<double>(0, 0)),
                centroid - cv::Point2f(100, 100), 1, 3, cv::Scalar(0, 0, 255),
                3);
    cv::putText(cv_image_->image, std::to_string(tvec.at<double>(0, 1)),
                centroid - cv::Point2f(50, 50), 1, 3, cv::Scalar(0, 0, 255), 3);
    cv::putText(cv_image_->image, std::to_string(tvec.at<double>(0, 2)),
                centroid - cv::Point2f(10, 10), 1, 3, cv::Scalar(0, 0, 255), 3);
    cv::putText(cv_image_->image, std::to_string(angle),
                centroid + cv::Point2f(50, 50), 1, 3, cv::Scalar(255, 0, 255),
                3);
    return tag_msg;
  }
...
int Tag::recognizeLetter(const cv::Mat &reverse_mask) {
  std::vector<std::pair<int, double>> similar_value_vec;
  cv::Mat tmp_a, tmp_b, tmp_c, tmp_d, tmp_e;
  cv::bitwise_xor(reverse_mask, binary_a_, tmp_a);
  cv::bitwise_xor(reverse_mask, binary_b_, tmp_b);
  cv::bitwise_xor(reverse_mask, binary_c_, tmp_c);
  cv::bitwise_xor(reverse_mask, binary_d_, tmp_d);
  cv::bitwise_xor(reverse_mask, binary_e_, tmp_e);
  ...
  std::sort(similar_value_vec.begin(), similar_value_vec.end(),
          [](std::pair<int, int> val1, std::pair<int, int> val2) {
              return val1.second < val2.second;
          });
  if ((similar_value_vec[0].second /
     (reverse_mask.rows * reverse_mask.cols)) >= area_thresh_) {
    return 6;
}
...
int Tag::findMatchPoint(const cv::Point2f &rotate_point, const std::vector<cv::Point2i> &approx_points)
{
    std::vector<std::pair<int,int>> dist_vec;
    int index = 0;
    for (auto &point : approx_points)
    {
...
rm_msgs::TagMsgArray Tag::contoursProcess(const cv::Mat *mor_ptr, int color) {
  ...
  for (int i = 0; i < hull_size; i++) {
    ...
    for (const auto &approx_point : approx_points)
      cv::circle(cv_image_->image, approx_point, 8, cv::Scalar(255, 0, 255), 3);
    ...
    cv::warpAffine(*mor_ptr, *warp_result, trans_matrix,
                   cv::Size(mor_ptr->cols, mor_ptr->rows));
    cv::getRectSubPix(*warp_result, rotate_rect.size, rotate_rect.center,
                      *mask);
    delete warp_result;
    ...
    cv::resize(reverse_mask, reverse_mask, cv::Size(40, 40), 0, 0,
               cv::INTER_NEAREST);
    if (color)
      masked_red_publisher_.publish(
          cv_bridge::CvImage(std_msgs::Header(), "mono8", reverse_mask)
              .toImageMsg());
    else
      masked_blue_publisher_.publish(
          cv_bridge::CvImage(std_msgs::Header(), "mono8", reverse_mask)
              .toImageMsg());
...
void Tag::imgProcess() {
  ...
  cv::inRange(*hsv_red_ptr,
              cv::Scalar(lower_red_hsv_h_, lower_red_hsv_s_, lower_red_hsv_v_),
              cv::Scalar(upper_red_hsv_h_, upper_red_hsv_s_, upper_red_hsv_v_),
              *binary_red_ptr);
  cv::inRange(
      *hsv_blue_ptr,
      cv::Scalar(lower_blue_hsv_h_, lower_blue_hsv_s_, lower_blue_hsv_v_),
      cv::Scalar(upper_blue_hsv_h_, upper_blue_hsv_s_, upper_blue_hsv_v_),
      *binary_blue_ptr);

  delete hsv_red_ptr;
  delete hsv_blue_ptr;
  cv::Mat kernel = cv::getStructuringElement(
      cv::MORPH_RECT, cv::Size(1 + 2 * morph_size_, 1 + 2 * morph_size_),
      cv::Point(-1, -1));
  cv::morphologyEx(*binary_red_ptr, *mor_red_ptr, morph_type_, kernel,
                   cv::Point(-1, -1), morph_iterations_);
  cv::morphologyEx(*binary_blue_ptr, *mor_blue_ptr, morph_type_, kernel,
                   cv::Point(-1, -1), morph_iterations_);

  delete binary_red_ptr;
  delete binary_blue_ptr;
  hsv_red_publisher_.publish(
      cv_bridge::CvImage(std_msgs::Header(), "mono8", *mor_red_ptr)
          .toImageMsg());
  hsv_blue_publisher_.publish(
  ...
  red_tag_msg.tag_msgs_array.insert(red_tag_msg.tag_msgs_array.end(),
                                    ...
                                    blue_tag_msg.tag_msgs_array.end());
  red_tag_msg.header.stamp = ros::Time::now();
